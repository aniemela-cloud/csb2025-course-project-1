...
def test_password(address, candidates):
  ...
  resp = s.get(f'{address}/users/login/')
  cstoken = extract_token(resp)
  # login attempt is something like
  for pw in candidates:
    resp = s.post(f'{address}/users/login/', data={'username': 'testuser', 'password': pw, 'csrfmiddlewaretoken':cstoken})
